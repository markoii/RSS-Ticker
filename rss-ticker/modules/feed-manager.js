		...
		req.onreadystatechange = function ( event ) {
			if ( req.readyState == 4 ) {
				if ( req.status == 200 ){
					...
					try {
						//if ( encoding_matches[1].toLowerCase() == "iso-8859-1" ) 
	...
	handleResult : function ( result ) {
		...
		if ( ! result.bozo ) {
			...
			if ( feed ) {
				...
				if ( feed.icon )
					feedObject.image = this.entityDecode( feed.icon.plainText() );
				else
				...
				for ( var i = 0, _len = feed.items.length; i < _len; i++ ) {
					...
					itemObject.time = item.fields.get( "atom:updated" );
					if ( !itemObject.time ) itemObject.time = item.fields.get( "atom:issued" );
					if ( !itemObject.time ) itemObject.time = item.fields.get( "atom:modified" );
					if ( !itemObject.time ) itemObject.time = item.fields.get( "atom:published" );
					if ( !itemObject.time ) itemObject.time = item.fields.get( "updated" );
					if ( !itemObject.time ) itemObject.time = item.fields.get( "pubDate" );
					if ( !itemObject.time ) itemObject.time = item.fields.get( "dc:date" );
					if ( !itemObject.time ) itemObject.time = item.fields.get( "published" );
					if ( !itemObject.time ) itemObject.time = "";
					...
					if ( itemObject.time.match ( /\d+ \d+ \d+/ ) ) {
						var monthName = monthsList[ Number( itemObject.time.substring( 3, 5 ) ) -1 ];
						itemObject.time = itemObject.time.substring( 0, 3 ) + monthName + itemObject.time.substring( 5 );
					}
					...
					if ( itemObject.url.match( /\/~r\//i ) ) {
						...
						if ( item.image )
							...
							itemObject.image = feedObject.image;
					}
